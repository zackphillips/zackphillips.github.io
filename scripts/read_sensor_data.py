...
class SensorReader:
    def __init__(self, signalk_host="192.168.8.50", signalk_port=3000):
        ...
        self.signalk_ws_url = f"ws://{signalk_host}:{signalk_port}/signalk/v1/stream?subscribe=self"
    ...
    def read_bno055_data(self):
        ...
        try:
            ...
            if not self.bno055_sensor.calibrated and not hasattr(self, '_bno055_calibration_warned'):
                logger.warning("BNO055 not calibrated - move sensor in figure-8 pattern to calibrate")
            ...
            # Read magnetometer
            mag = self.bno055_sensor.magnetic
            # Read euler angles
            euler = self.bno055_sensor.euler
            # Read quaternion
            quat = self.bno055_sensor.quaternion
            data = {}
    ...
    def _handle_hello_message(self):
        ...
        try:
            ...
            if "self" in hello_data:
                ...
                logger.info(f"Received hello from SignalK server: {hello_data.get('name', 'Unknown')} v{hello_data.get('version', 'Unknown')}")
    ...
    def publish_to_signalk(self, data):
        ...
        try:
            ...
            if self.ws and self.ws_connected:
                ...
                logger.info(f"Published {len(data)} data points to SignalK via WebSocket")
            else:
...
def main():
    ...
    parser.add_argument('--test', action='store_true', help='Test SignalK connection only')
