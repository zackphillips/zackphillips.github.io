...
11. [Vessel Data / Automation](#11-vessel-data-automation)
...
- The same BLE plugin also reads a second Victron device labeled "bimini" (solar charge controller) — see the Solar row below; this hasn't yet been reconciled with the documented Kyocera/Blue Sky Solar Boost setup, so confirm whether it's an additional panel or a controller swap.
...
| Solar | 120 W | One Kyocera KC120-1 panel, Blue Sky Solar Boost controller/regulator. A second Victron charge controller (labeled "bimini" in SignalK, reporting over BLE) is also present — not yet reconciled with this row; confirm whether it's a separate bimini-mounted panel. |
...
- **Superseded**: TackTick (MN-100-2 solar wind display) — displayed
  ...
  not been proven reliable. The Tacktick wireless pack is still wired into
  SignalK over NMEA 0183 serial (`/dev/ttyOP_tacktick`, 4800 baud), but
  SignalK is explicitly configured to **ignore** its wind/nav sentences
  (RMC/RMB/GLL/MWV/VWR) — so the hardware is present but not the active
  source.
