...
def test_tws_bin_snaps_up():
    assert (
        upd._tws_bin(11.0) == 12
    )  # rounds to nearest 2 (11 rounds to 12 for nearest even)
...
def test_write_polar_csv_row_count(tmp_path):
    ...
    csv = tmp_path / "polars_calculated.csv"
    upd.write_polar_csv(acc, csv)
    data_lines = [line for line in csv.read_text().strip().split("\n") if line]
    # 1 header + 9 TWA rows
...
def test_write_polar_csv_twa_rows(tmp_path):
    ...
    twas = [int(line.split(";")[0]) for line in lines[1:] if line]
...
def test_write_polar_csv_reflects_bin_data(tmp_path):
    ...
    lines = csv.read_text().strip().split("\n")
    row_90 = next(line for line in lines if line.startswith("90;"))
...
def test_fetch_polar_sample_returns_tuple():
    with patch(
        "scripts.update_polar_data.requests", _fake_requests(_make_signalk_blob())
    ):
