...
def test_integration_updates_test_branch(tmp_path, test_branch):
    ...
    with patch.dict(sys.modules, {"requests": FakeRequests}):
        ...
        # Fetch the ref hash
        before_after = (
            subprocess.check_output(["git", "ls-remote", str(origin), f"refs/heads/{test_branch}"])
            .decode()
            .strip()
        )
        assert before_after, f"{test_branch} branch should exist on origin and have a commit"
