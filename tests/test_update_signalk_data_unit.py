...
def test_run_on_dev_branch_writes_file_and_calls_git(tmp_path):
...
def test_https_conversion(tmp_path):
...
def test_no_reset_mode(tmp_path):
...
def test_amend_commit(tmp_path):
...
def test_no_push_mode(tmp_path):
...
def test_requests_error_handling(tmp_path):
